...
export const generateAIFeedback = (
  ...
  cyclomaticRating: any,
  maintainabilityRating: any,
  reliabilityRating: any,
  testCases: TestCase[],
...
${cyclomaticRating.reason}

${cyclomaticComplexityScore > 10 ? 
`**Issues Found:**
${cyclomaticRating.issues?.map((issue: string) => `- ${issue}`).join('\n') || ''}

**Improvement Recommendations:**
${cyclomaticRating.improvements?.map((imp: string) => `- ${imp}`).join('\n') || ''}` : 
...
${maintainabilityRating.reason}

${maintainabilityScore < 85 ? 
`**Issues Found:**
${maintainabilityRating.issues?.map((issue: string) => `- ${issue}`).join('\n') || ''}

**Improvement Recommendations:**
${maintainabilityRating.improvements?.map((imp: string) => `- ${imp}`).join('\n') || ''}` : 
...
${reliabilityRating.reason}

${reliabilityScore < 85 ? 
`**Issues Found:**
${reliabilityRating.issues?.map((issue: string) => `- ${issue}`).join('\n') || ''}

**Improvement Recommendations:**
${reliabilityRating.improvements?.map((imp: string) => `- ${imp}`).join('\n') || ''}` : 
'**Well done!** Your code handles errors appropriately and includes proper input validation.'}

## Code Metrics
- **Lines of Code:** ${metrics.linesOfCode}
- **Comment Coverage:** ${metrics.commentPercentage.toFixed(1)}%
...
${metrics.commentPercentage >= 15 ? "- Well-documented code with good comment coverage\n" : ""}
${cyclomaticComplexityScore <= 10 ? "- Clean control flow with reasonable complexity\n" : ""}
${metrics.functionCount > 0 && metrics.averageFunctionLength <= 15 ? "- Functions are appropriately sized and focused\n" : ""}
`;
};
